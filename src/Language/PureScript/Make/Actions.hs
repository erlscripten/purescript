...
buildMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  getOutputTimestamp mn = do
    codegenTargets <- asks $ optionsCodegenTargets . J.options
  ...
  codegen m docs exts = do
    ...
    codegenTargets <- lift $ asks $ optionsCodegenTargets . J.options
    ...
    when (S.member JS codegenTargets) $ do
      ...
      let sourceMaps = S.member JSSourceMap codegenTargets
          ...
          prefix = ["Generated by purs version " <> T.pack (showVersion Paths.version) <> ". Patched for erlscripten!" | usePrefix]
