...
import Language.PureScript.Bundle.Types
import qualified Language.PureScript.CoreImp.AST as CoreAST
...
data ModuleElement
  ...
  | Block (Maybe String) [JSStatement] [Key]
  | Other JSStatement
  | Skip JSStatement
  deriving (Show)
...
instance A.ToJSON ModuleElement where
  toJSON = \case
    ...
    (Block name stmts dependsOn) ->
      A.object [ "statemtents" .= A.toJSON (map getFragment stmts)
               , "name" .= A.toJSON name
               , "dependsOn"  .= map keyToJSON dependsOn
               ]
    (Other stmt) ->
...
withDeps (Module modulePath fn es) = Module modulePath fn (map expandDeps es)
  ...
  boundNames :: [String]
  boundNames = concatMap toBoundName es
    where
    toBoundName :: ModuleElement -> [String]
    toBoundName (Member _ Internal nm _ _) = [nm]
    toBoundName _ = []
  ...
  expandDeps (Block n b _) = Block n b keys where
    keys = ordNub $
      dependencies modulePath
      (JSFunctionExpression JSNoAnnot JSIdentNone JSNoAnnot JSLNil JSNoAnnot
        (JSBlock JSNoAnnot b JSNoAnnot))
...
toModule mids mid filename top
  ...
  toModuleElement stmt
    | Just (name, block) <- matchBlock stmt
    = pure (Block name block [])
  toModuleElement stmt
...
matchBlock :: JSStatement -> Maybe (Maybe String, [JSStatement])
matchBlock (JSLabelled (JSIdentName _ label) _ block)
  | Just (Nothing, matched) <- matchBlock block
  = Just (Just label, matched)
matchBlock (JSStatementBlock _ block _ _) = Just (Nothing, block)
...
compile modules entryPoints = filteredModules
  ...
  verts = do
    ...
    toVertices p m@(Block (Just init_name) _ deps)
      | Just n <- CoreAST.dropInitializerName (TS.pack init_name)
      = [(m, (p, TS.unpack n, Public), deps)]
  ...
  filteredModules = map filterUsed modules
    ...
    filterUsed (Module mid fn ds) =  trace ("REFERENCED: " <> show (fold $ M.lookup mid moduleReferenceMap)) $  Module mid fn (map filterExports (go ds))
      ...
      skipDecl (Block _ s _) = map Skip s
...
codeGen optionsMainModule optionsNamespace ms outFileOpt = (fmap sourceMapping outFileOpt, rendered)
  ...
  moduleToJS (Module mid _ ds) = (wrap mid (indent (concat jsDecls)), lengths)
    ...
    declToJS (Block Nothing b _) = withLength [JSStatementBlock JSAnnotSpace b JSAnnotSpace JSSemiAuto]
    declToJS (Block (Just name) b _) = withLength [JSLabelled (JSIdentName JSAnnotSpace name) JSAnnotSpace (JSStatementBlock JSAnnotSpace b JSAnnotSpace JSSemiAuto)]
  ...
  prelude :: JSStatement
  prelude = JSVariable (JSAnnot tokenPosnEmpty [ CommentA tokenPosnEmpty $ "// Generated by purs bundle " ++ showVersion Paths.version ++ ". Erlscripten edition!"
